...
import {
  DICCIONARIO_FORMULARIO_DEVOLUCIONES,
...
export default function FormularioDevolucion({
  ...
  const { ObtenerInformacionDeGuiaParaDevolucion } = useBodega();
  ...
  const BuscarInformacionDeGuiaParaDevolucion = handleSubmit(async (info) => {
    ...
    // VERIFICAMOS QUE LA GUIA NO EXISTA EN LA DEVOLUCION ACTUAL
    if (devolucion.some((guia) => guia.GuiaPedido === valorInput)) {
      toast.error(`¡La guía ${valorInput} ya se encuentra en la devolución!`, {
    ...
    try {
      ...
      const res = await ObtenerInformacionDeGuiaParaDevolucion(info);
      if (res.response) {
        const { status, data } = res.response;
        ManejarMensajesDeRespuesta({ status, data });
      } else {
        toast.success(
          `¡La guía ${info.GuiaPedido} ha sido agregada con éxito a la devolución!`,
          {
            style: ESTILOS_SUCCESS,
          }
        );
        AgregarNuevaGuiaDevolucion(res.data);
      }
    } catch (error) {
  ...
  const AgregarNuevaGuiaDevolucion = (infGuia) => {
    establecerDevolucion([infGuia, ...devolucion]);
    ReiniciarValores();
  };
  ...
  return (
    <form
      className="FormularioDevolucion"
